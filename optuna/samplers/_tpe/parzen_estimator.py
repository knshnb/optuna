    ...
    def _calculate_categorical_distributions(
        ...
        if param_name in parameters.categorical_distance_func:
            ...
            for i, observation in enumerate(observations.astype(int)):
                ...
                if "max-norm" in parameters.categorical_distance_func:
                    exponent = -((np.array(dists) / max(dists)) ** 2)
                else:
                    exponent = -(np.array(dists) ** 2)
                if "trial-decay" in parameters.categorical_distance_func:
                    exponent *= np.log((len(observations) + consider_prior) / parameters.prior_weight)
                if "log-base" in parameters.categorical_distance_func:
                    exponent *= np.log(len(search_space.choices)) / np.log(parameters.categorical_distance_func["log-base"])
